...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        if self.height_on:
            # A shared bottleneck MLP adds only ~16.5K parameters at d=256.
            self.height_embed = MLP(
                hidden_dim, max(hidden_dim // 4, 1), 1, 2
            )
            nn.init.constant_(self.height_embed.layers[-1].weight.data, 0)
            nn.init.constant_(self.height_embed.layers[-1].bias.data, 0)
        else:
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        # The SAR baseline predicts height only after its separate geometry
        # decoder. Keep the legacy optical-only head available for old configs.
        ret["height_on"] = (
            cfg.MODEL.MaskDINO.HEIGHT_ON and not cfg.MODEL.SAR.ENABLED
        )
    ...
    def dn_post_process(
        ...
        if outputs_height is not None:
            output_known_height = outputs_height[:, :, :mask_dict['pad_size'], :]
            outputs_height = outputs_height[:, :, mask_dict['pad_size']:, :]
            out['pred_heights'] = output_known_height[-1]
        else:
            output_known_height = None
        ...
        return outputs_class, outputs_coord, outputs_mask, outputs_height
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            outputs_class, outputs_mask, outputs_height = self.forward_prediction_heads(
            ...
            if outputs_height is not None:
                interm_outputs['pred_heights'] = outputs_height
        ...
        if self.initial_pred:
            ...
            if outputs_height is not None:
                predictions_height.append(outputs_height)
        ...
        if mask_dict is not None:
            ...
            predictions_height = (
                torch.stack(predictions_height) if self.height_on else None
            )
            predictions_class, out_boxes, predictions_mask, predictions_height = \
            ...
            if predictions_height is not None:
                predictions_height = list(predictions_height)
        ...
        out = {
            ...
            'aux_outputs': self._set_aux_loss(
                ...
                predictions_mask,
                out_boxes,
                predictions_height if self.height_on else None,
            )
        }
        if self.height_on:
            out['pred_heights'] = predictions_height[-1]
        final_decoder_query = hs[-1]
        if mask_dict is not None:
            final_decoder_query = final_decoder_query[:, mask_dict['pad_size']:, :]
        out['maskdino_features'] = {
            'decoder_query': final_decoder_query,
            # The final predicted box is normalized cx/cy/w/h and corresponds
            # one-to-one with the exposed final query.
            'reference_boxes': out_boxes[-1],
        }
        if self.two_stage:
    ...
    def forward_prediction_heads(self, output, mask_features, pred_mask=True):
        ...
        outputs_height = (
            self.height_embed(decoder_output) if self.height_embed is not None else None
        )
        outputs_mask = None
        if pred_mask:
        ...
        return outputs_class, outputs_mask, outputs_height

    @torch.jit.unused
    def _set_aux_loss(
        self, outputs_class, outputs_seg_masks, out_boxes=None, outputs_height=None
    ):
        ...
        # if self.mask_classification:
        aux_outputs = []
        for i, (outputs_class_i, outputs_mask_i) in enumerate(
            zip(outputs_class[:-1], outputs_seg_masks[:-1])
        ):
            output = {
                "pred_logits": outputs_class_i,
                "pred_masks": outputs_mask_i,
            }
            if out_boxes is not None:
                output["pred_boxes"] = out_boxes[i]
            if outputs_height is not None:
                output["pred_heights"] = outputs_height[i]
