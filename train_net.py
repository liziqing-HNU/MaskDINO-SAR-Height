...
from maskdino import (
    ...
    add_maskdino_config,
    DetrDatasetMapper,
    BuildingHeightCOCOEvaluator,
)
from maskdino.data.datasets.register_sp6_building import register_sp6_building
import random
        ...
        if evaluator_type == "coco_height":
            evaluator_list.append(
                BuildingHeightCOCOEvaluator(
                    ...
                    output_dir=output_folder,
                    max_dets_per_image=cfg.TEST.DETECTIONS_PER_IMAGE,
                    height_score_threshold=(
                        cfg.MODEL.MaskDINO.TEST.HEIGHT_SCORE_THRESHOLD
                    ),
                    height_iou_threshold=(
                        cfg.MODEL.MaskDINO.TEST.HEIGHT_IOU_THRESHOLD
                    ),
                )
            )
...
def setup(args):
    ...
    if cfg.DATASETS.SP6_ROOT:
        register_sp6_building(
            root=os.getenv("SP6_DATASET_ROOT", cfg.DATASETS.SP6_ROOT),
            annotation_file=os.getenv(
                "SP6_ANNOTATION_FILE", cfg.DATASETS.SP6_ANNOTATION_FILE
            ),
            split_ratios=cfg.DATASETS.SP6_SPLIT_RATIOS,
            split_seed=cfg.DATASETS.SP6_SPLIT_SEED,
            max_train_images=cfg.DATASETS.SP6_MAX_TRAIN_IMAGES,
        )
    cfg.freeze()
