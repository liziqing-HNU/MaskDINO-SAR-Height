...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        self.height_loss_beta = height_loss_beta
    ...
    def loss_heights(self, outputs, targets, indices, num_boxes):
        """
        Regress one normalized height for each matched instance.

        Invalid/missing height annotations still supervise class, box and mask,
        but are excluded from this loss.
        """
        assert "pred_heights" in outputs
        idx = self._get_src_permutation_idx(indices)
        src_heights = outputs["pred_heights"][idx]
        target_heights = torch.cat(
            [
                target["heights"][target_idx]
        ...
        target_valid = torch.cat(
            [
                target["height_valid"][target_idx]
                for target, (_, target_idx) in zip(targets, indices)
            ],
            dim=0,
        ).bool()
        ...
        if target_valid.any():
            loss_height = F.smooth_l1_loss(
                src_heights[target_valid],
                target_heights[target_valid] / self.height_scale,
                reduction="mean",
                beta=self.height_loss_beta,
            )
        else:
            # Keep the graph connected on batches with no valid height labels.
            loss_height = src_heights.sum() * 0.0
        return {"loss_height": loss_height}
    ...
    def forward(
        ...
        requested_losses = (
            self.losses
            if compute_maskdino_losses
            else [loss for loss in self.losses if loss == "heights"]
        )
        for loss in requested_losses:
            losses.update(self.get_loss(loss, outputs, targets, indices, num_masks))

        if compute_maskdino_losses and self.dn != "no" and mask_dict is not None:
            l_dict={}
        ...
        elif compute_maskdino_losses and self.dn != "no":
            l_dict = dict()
        ...
        if compute_maskdino_losses and "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                indices = self.matcher(aux_outputs, targets)
                for loss in self.losses:
                    if loss == "heights" and "pred_heights" not in aux_outputs:
                        continue
                ...
                if i>=start:
                    if self.dn != "no" and mask_dict is not None:
                        ...
                        losses.update(l_dict)
        # interm_outputs loss
        if compute_maskdino_losses and 'interm_outputs' in outputs:
            interm_outputs = outputs['interm_outputs']
            indices = self.matcher(interm_outputs, targets)
            for loss in self.losses:
                if loss == "heights" and "pred_heights" not in interm_outputs:
                    continue
