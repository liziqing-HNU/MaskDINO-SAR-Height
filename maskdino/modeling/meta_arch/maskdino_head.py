...
class MaskDINOHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        stride8_features = [name for name, shape in input_shape if shape.stride == 8]
        if not stride8_features:
            raise ValueError("MaskDINO SAR interface requires a stride-8 backbone feature")
        self.stride8_feature_name = stride8_features[0]
    ...
    def layers(self, features, mask=None,targets=None):
        ...
        predictions, mask_dict = self.predictor(
            multi_scale_features, mask_features, mask, targets=targets
        )
        stride8_size = features[self.stride8_feature_name].shape[-2:]
        stride8_candidates = [
            feature
            for feature in multi_scale_features
            if feature.shape[-2:] == stride8_size
        ]
        if len(stride8_candidates) != 1:
            raise RuntimeError(
                "could not identify one projected stride-8 pixel-decoder feature; "
                f"target={stride8_size}, candidates="
                f"{[feature.shape[-2:] for feature in multi_scale_features]}"
            )
        predictions["maskdino_features"]["optical_feature_s8"] = (
            stride8_candidates[0]
        )
